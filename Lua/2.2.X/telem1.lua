...
local function drawSpeed(x,y)
  ...
  lcd.drawNumber(x + 37, y, speed / 36 * 1000, MIDSIZE+RIGHT)
...
local function drawAltitude(x, y)
  ...
  lcd.drawNumber(x + 36, y, altitude, MIDSIZE+RIGHT)
  local t = lcd.getLastPos() + 1
...
local function drawDistance(x, y)
  ...
  if (type(coords) == "table" and type(arming_coords) == "table" and arming_state ~= 0) then
    ...
    if distance >= 100.0 then
      ...
      lcd.drawNumber(x + 36, y, distance, MIDSIZE + PREC2+RIGHT)
      local t = lcd.getLastPos() + 1
      lcd.drawText(t, y + 5, "Km", SMLSIZE)
    else
      lcd.drawNumber(x + 36, y, distance, MIDSIZE+RIGHT)
      local t = lcd.getLastPos() + 1
      lcd.drawText(t, y + 5, "m", SMLSIZE)
    end
else
...
local function drawSats(x, y)
  ...
  if lock >= 3 then
    lcd.drawNumber(x + 6, y, numSats, MIDSIZE+RIGHT)
...
local function drawTopPanel()
  ...
  lcd.drawNumber(lcd.getLastPos() + 16, 1, getValue("tx-voltage") * 10, PREC1+INVERS+RIGHT)

  lcd.drawText(lcd.getLastPos(), 1, "V", INVERS)

  lcd.drawText(lcd.getLastPos() + 10, 1, "RSSI:", INVERS)
  lcd.drawNumber(lcd.getLastPos() + 16, 1, getValue("RSSI"), 0+INVERS+RIGHT)
end
--
local function drawBottomPanel()
  ...
  if getTime() < (messageLatestTimestamp + 1000) then
    ...
    lcd.drawNumber(lcd.getLastPos() + 16, 55, getHeading(), INVERS+RIGHT)
  end
end
--
